...
- **Список спутников** — ручное добавление NORAD ID
- **Импорт TLE** — добавление спутников из `.tle`/текстового файла
- **RX/TX библиотека** — автоматическое подтягивание частот в карточке спутника (если есть в базе)
- **Метки частот** — комментарий к каждой частоте, пометка «не работает», удаление из списка и восстановление
...
│   ├── SatelliteFrequencyLibrary.swift   # Библиотека RX/TX частот (из xlsx)
│   ├── SatelliteFrequencyStateStore.swift # Комментарии/метки/скрытие частот (UserDefaults)
